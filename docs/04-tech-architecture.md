...
| slug | テキスト | |
| thumbnail | 画像 | |
| workType | セレクト | Web制作／グラフィックデザイン／名刺制作／サポート事例（初期掲載件数: 各4〜5件・4〜5点・5〜6点・2件、[decisions.md#d006](./decisions.md#d006)参照） |
| industry | セレクト | 業種 |
...
> ✅ **予算方針との整合確認済み**（2026-07-06）: microCMSは無料プラン（Hobby）で契約済み。公式料金ページ確認の結果、無料プランでもAPI数上限は5個・コンテンツ数上限10,000件・APIコール数無制限であり、本設計の4コンテンツモデル（blog/works/news/faq）は問題なく収まる。予算方針（サーバー代+AI利用料のみ）と矛盾しない（[decisions.md#d009](./decisions.md#d009)）。
> ただし1メディアあたり画像サイズ上限40MB、独自ドメイン非対応（Enterpriseプランのみ）という制約はあるため、画像は事前に圧縮してアップロードする運用とする。
