...
>
> ⚠️ **予算方針との整合確認が必要**: 予算は「サーバー代とAI利用料程度」に抑えたいとのご要望（[decisions.md#d006](./decisions.md#d006)）。本設計はblog/works/news/faqの4コンテンツモデルを想定しているが、microCMSの無料プランでこの数が収まるかは要確認。収まらない場合は月額費用が発生するため、契約前に必ず料金ページで上限を確認し、収まらない場合はコンテンツモデルの統合（例: news と faq を1つのAPIにまとめる等）も検討する。
