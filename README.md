# みけねこデザイン コーポレートサイト

営業ツール／ブランディング／SEO資産／AI運用実験場／将来的なAI社員による運用対象、という5つの役割を持つコーポレートサイトの制作プロジェクト。

## ドキュメント
- [プロジェクト概要](docs/00-project-overview.md)
- [要件定義](docs/01-requirements.md)
- [サイト構成・IA](docs/02-site-structure.md)
- [SEO設計・コンテンツ戦略](docs/03-seo-content-strategy.md)
- [技術スタック・ディレクトリ設計](docs/04-tech-architecture.md)
- [Git/GitHub運用](docs/05-git-github-operations.md)
- [更新フロー・AI運用ガイド](docs/06-operation-guide.md)
- [デプロイ戦略](docs/07-deploy-strategy.md)
- [デザインガイド](docs/08-design-guide.md)
- [コンテンツ・素材準備チェックリスト](docs/09-content-checklist.md)
- [決定事項ログ](docs/decisions.md)
- [変更履歴](docs/CHANGELOG.md)

## 現在のフェーズ
Phase1: 設計・仕様策定（実装未着手）

## 元の要望メモ
[Project.md](Project.md)
